  ...
  <el-container>
    <el-main>
      ...
      <div class="select-wrap">
        <span>所属项目：</span>
        <el-select v-model="projectSelected" @change="getPlansFunc" :disabled="!(pagePermission.get)">
          <el-option
      ...
      <div v-loading="isPlanTableLoading" class="table-wrap">
        <el-table ref="planTable" :data="planTableData">
          ...
          <el-table-column width="250" label="操作" align="center">
            <template slot-scope="scope" width="180">
              <el-button size="mini" type="primary" @click="editPlanHandle(scope.row)" :disabled="!(pagePermission.update)">编辑</el-button>
              <el-button size="mini" type="primary" @click="delPlanHandle(scope.row)" :disabled="!(pagePermission.delete)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
export default {
  components: { AddPlan },
  mixins: [planTypeMixin, permissionOfPage],
  data() {
    return {
      projectSelected: "",
      projectStageList: [],
  ...
  methods: {
    ...mapActions(["getProjectDetailsVuex"]),
    // 平面图表格的平面图类型数据格式化
