          ...
          <el-table-column width="120" label="操作" align="center">
            <template slot-scope="scope">
              <el-button
                size="mini"
                type="primary"
                class="post-table-btn"
                :disabled="!(pagePermission.delete)"
                @click.stop="unBindParticipant(scope.row)"
              >移除</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  mixins: [getProjectMixin, permissionOfPage],
  data() {
    return {
  ...
  methods: {
    ...
    getParticipants(val) {
      ...
      this.getProjectParticipants(_obj)
        .then(data => {
    ...
    orgTypeformatter(row, column) {
      let backVal;
      const target = this.suppliersTypeData.find(
        item => item.id === row.orgType
      );
    ...
    particiDetailHandle(row, column, event) {
      if (!this.pagePermission.add) {
        this.$alert('您没有查看的权限')
        return false
      }
      const curProject = this.projectDetailDatas.find(
        item => item.id === this.projectSelected
      );
      const params = {
    ...
    unBindParticipant(row) {
      ...
      this.$confirm(
        `确定将 ${row.fullName} 移出 ${curProject.name} 项目？`,
        "警告",
        {
